...
import 'killer_sudoku_badge_definitions.dart';

/// 킬러 스도쿠 배지 서비스 — 달성 평가 및 저장
class KillerSudokuBadgeService {
  static const _keyAcquiredBadges = 'killer_sudoku_acquired_badges';
  final SharedPreferences _prefs;

  KillerSudokuBadgeService(this._prefs);
  ...
  List<BadgeDefinition> evaluateNewBadges(List<CompletedGameRecord> records) {
    ...
    for (final badge in killerSudokuBadgeDefinitions) {
  ...
  List<({BadgeDefinition badge, bool acquired})> getAllBadges() {
    final acquired = getAcquiredBadgeIds();
    return killerSudokuBadgeDefinitions
