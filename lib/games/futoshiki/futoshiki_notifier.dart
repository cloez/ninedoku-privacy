...
import 'futoshiki_badge_service.dart';
import 'futoshiki_storage_service.dart';
import 'engine/futoshiki_board.dart';
import 'engine/futoshiki_generator.dart';
...
const _storageKey = 'futoshiki_current_game';

/// 후토시키 게임 상태 관리 Notifier
class FutoshikiNotifier extends StateNotifier<FutoshikiState?>
    with WidgetsBindingObserver {
  Timer? _timer;
  final SharedPreferences? _prefs;

  /// 완료 기록 저장 서비스
  FutoshikiStorageService? _storageService;

  /// 배지 평가 서비스
  FutoshikiBadgeService? _badgeService;

  /// 마지막으로 새로 획득한 배지 목록 (결과 화면 표시용)
  List<BadgeDefinition> lastNewBadges = [];

  FutoshikiNotifier({SharedPreferences? prefs})
      : _prefs = prefs,
        super(null) {
    if (prefs != null) {
      _storageService = FutoshikiStorageService(prefs);
      _badgeService = FutoshikiBadgeService(prefs);
    }
  ...
  void _tryRestore() {
    ...
    try {
      ...
      final saved = FutoshikiState.fromJson(json);
  ...
  /// 새 게임 시작
  void startNewGame({
    required FutoshikiGameMode mode,
    required FutoshikiDifficulty difficulty,
  }) {
    _timer?.cancel();
    lastNewBadges = [];
  ...
  void startDailyPuzzle() {
    ...
    const difficulty = FutoshikiDifficulty.medium;
    ...
    state = FutoshikiState(
      ...
      solution: result.solution,
      current: result.puzzle.copyWith(),
      mode: FutoshikiGameMode.dailyPuzzle,
      difficulty: difficulty,
    );
  ...
  void _toggleNote(int row, int col, int num) {
    ...
    final undoAction = FutoshikiUndoAction(
      type: FutoshikiUndoType.toggleNote,
      row: row,
      col: col,
      previousValue: 0,
  ...
  void _applyValue(int row, int col, int previousValue, int newValue) {
    ...
    final actionType = newValue == 0
        ? FutoshikiUndoType.clearValue
        : FutoshikiUndoType.setValue;
    final undoAction = FutoshikiUndoAction(
      type: actionType,
      row: row,
      col: col,
  ...
  void undo() {
    ...
    FutoshikiBoard newBoard;

    if (lastAction.type == FutoshikiUndoType.toggleNote) {
      // 메모 되돌리기
  ...
  void getHint() {
    ...
    final hint = FutoshikiHintEngine.getHint(
      state!.current,
      state!.solution,
      level: nextLevel,
    );
  ...
  void _checkCompletion() {
    if (state == null) return;
    if (FutoshikiSolver.isComplete(state!.current)) {
      _timer?.cancel();
...
/// 후토시키 Provider
final futoshikiProvider =
    StateNotifierProvider<FutoshikiNotifier, FutoshikiState?>((ref) {
  return FutoshikiNotifier();
});

/// SharedPreferences가 주입된 FutoshikiNotifier
final futoshikiNotifierProvider =
    StateNotifierProvider<FutoshikiNotifier, FutoshikiState?>((ref) {
  try {
    final prefs = ref.watch(sharedPreferencesProvider);
    return FutoshikiNotifier(prefs: prefs);
  } catch (_) {
    return FutoshikiNotifier();
  }
});
