...
import 'futoshiki_badge_definitions.dart';

/// 후토시키 배지 서비스 — 달성 평가 및 저장
class FutoshikiBadgeService {
  static const _keyAcquiredBadges = 'futoshiki_acquired_badges';
  final SharedPreferences _prefs;

  FutoshikiBadgeService(this._prefs);
  ...
  List<BadgeDefinition> evaluateNewBadges(List<CompletedGameRecord> records) {
    ...
    for (final badge in futoshikiBadgeDefinitions) {
  ...
  List<({BadgeDefinition badge, bool acquired})> getAllBadges() {
    final acquired = getAcquiredBadgeIds();
    return futoshikiBadgeDefinitions
