...
import 'binairo_badge_definitions.dart';

/// 비나이로 배지 서비스 — 달성 평가 및 저장
class BinairoBadgeService {
  static const _keyAcquiredBadges = 'binairo_acquired_badges';
  final SharedPreferences _prefs;

  BinairoBadgeService(this._prefs);
  ...
  List<BadgeDefinition> evaluateNewBadges(List<CompletedGameRecord> records) {
    ...
    for (final badge in binairoBadgeDefinitions) {
  ...
  List<({BadgeDefinition badge, bool acquired})> getAllBadges() {
    final acquired = getAcquiredBadgeIds();
    return binairoBadgeDefinitions
