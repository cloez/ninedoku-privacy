...
import 'light_up_badge_service.dart';
import 'light_up_storage_service.dart';
import 'engine/light_up_board.dart';
import 'engine/light_up_generator.dart';
import 'engine/light_up_hint.dart';
import 'engine/light_up_solver.dart';
import 'light_up_state.dart';

import '../../shared/services/sound_manager.dart';
/// SharedPreferences 저장 키
const _storageKey = 'lightup_current_game';

/// Light Up 게임 상태 관리 Notifier
class LightUpNotifier extends StateNotifier<LightUpState?> with WidgetsBindingObserver {
  Timer? _timer;
  final SharedPreferences? _prefs;

  /// 완료 기록 저장 서비스
  LightUpStorageService? _storageService;

  /// 배지 평가 서비스
  LightUpBadgeService? _badgeService;

  /// 마지막으로 새로 획득한 배지 목록 (결과 화면 표시용)
  List<BadgeDefinition> lastNewBadges = [];

  LightUpNotifier({SharedPreferences? prefs})
      : _prefs = prefs,
        super(null) {
    if (prefs != null) {
      _storageService = LightUpStorageService(prefs);
      _badgeService = LightUpBadgeService(prefs);
    }
  ...
  void _tryRestore() {
    ...
    try {
      ...
      final saved = LightUpState.fromJson(json);
  ...
  /// 새 게임 시작
  void startNewGame({
    required LightUpGameMode mode,
    required LightUpDifficulty difficulty,
  }) {
    _timer?.cancel();
    lastNewBadges = [];
  ...
  void startDailyPuzzle() {
    ...
    const difficulty = LightUpDifficulty.medium;
    ...
    state = LightUpState(
      ...
      solution: result.solution,
      current: result.puzzle.copyWith(),
      mode: LightUpGameMode.dailyPuzzle,
      difficulty: difficulty,
    );
  ...
  /// 입력 모드 변경 (💡 / X / 지우개)
  void setInputMode(LightUpInputMode mode) {
    if (state == null) return;
  ...
  void tapCell(int row, int col) {
    ...
    if (state!.current.fixed.contains(idx)) return; // 벽은 탭 불가

    final currentValue = state!.current.getValue(row, col);

    switch (state!.inputMode) {
      case LightUpInputMode.bulb:
        // 이미 전구면 제거, 아니면 전구 배치
        if (currentValue == LightUpBoard.bulb) {
          _applyValue(row, col, currentValue, LightUpBoard.empty);
        } else {
          _applyValue(row, col, currentValue, LightUpBoard.bulb);
        }
      case LightUpInputMode.crossMark:
        // 이미 X면 제거, 아니면 X 배치
        if (currentValue == LightUpBoard.cross) {
          _applyValue(row, col, currentValue, LightUpBoard.empty);
        } else {
          _applyValue(row, col, currentValue, LightUpBoard.cross);
        }
      case LightUpInputMode.erase:
        if (currentValue != LightUpBoard.empty) {
          _applyValue(row, col, currentValue, LightUpBoard.empty);
        }
    }
  }
  ...
  void toggleCell(int row, int col) {
    ...
    int newValue;
    switch (currentValue) {
      case LightUpBoard.empty:
        newValue = LightUpBoard.bulb;
      case LightUpBoard.bulb:
        newValue = LightUpBoard.cross;
      case LightUpBoard.cross:
        newValue = LightUpBoard.empty;
      default:
        newValue = LightUpBoard.empty;
    }
  ...
  void _applyValue(int row, int col, int previousValue, int newValue) {
    ...
    // 실수 판정: 전구를 놓았는데 정답에 전구가 없으면 실수
    var newMistakeCount = state!.mistakeCount;
    if (newValue == LightUpBoard.bulb) {
      final correctValue = state!.solution.getValue(row, col);
      if (correctValue != LightUpBoard.bulb) {
        newMistakeCount++;
        // 햅틱: 실수 강한 진동
    ...
    // Undo 스택에 추가
    final actionType = newValue == LightUpBoard.empty
        ? LightUpUndoActionType.clearValue
        : LightUpUndoActionType.setValue;
    final undoAction = LightUpUndoAction(
      type: actionType,
      row: row,
      col: col,
  ...
  void getHint() {
    ...
    final hint = LightUpHintEngine.getHint(
      state!.current,
      state!.solution,
      level: nextLevel,
    );
  ...
  void _checkCompletion() {
    if (state == null) return;
    if (LightUpSolver.isComplete(state!.current)) {
      _timer?.cancel();
...
/// Light Up Provider
final lightUpProvider = StateNotifierProvider<LightUpNotifier, LightUpState?>((ref) {
  return LightUpNotifier();
});

/// SharedPreferences가 주입된 LightUpNotifier
final lightUpNotifierProvider = StateNotifierProvider<LightUpNotifier, LightUpState?>((ref) {
  try {
    final prefs = ref.watch(sharedPreferencesProvider);
    return LightUpNotifier(prefs: prefs);
  } catch (_) {
    return LightUpNotifier();
  }
});
