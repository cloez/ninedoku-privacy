...
import '../widgets/light_up_board_widget.dart';
...
/// Light Up 게임 플레이 화면
class LightUpGameScreen extends ConsumerStatefulWidget {
  const LightUpGameScreen({super.key});

  @override
  ConsumerState<LightUpGameScreen> createState() => _LightUpGameScreenState();
}

class _LightUpGameScreenState extends ConsumerState<LightUpGameScreen> {
  @override
  Widget build(BuildContext context) {
    ...
    if (gameState == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) context.go(AppRoutes.lightUp);
      });
    ...
    if (gameState.isCompleted && !gameState.isAutoCompleting) {
      return _LightUpResultView(gameState: gameState);
    }

    // 일시정지 시
    if (gameState.isPaused) {
      return _LightUpPauseView(gameState: gameState);
    }
        ...
        appBar: isLandscape
            ...
            : AppBar(
                ...
                leading: IconButton(
                  ...
                  tooltip: AppStrings.get('lightUp.back'),
                ),
                actions: [
                  IconButton(
                    icon: const Icon(Icons.pause_rounded),
                    onPressed: () => ref.read(lightUpNotifierProvider.notifier).pause(),
                    tooltip: AppStrings.get('lightUp.pause'),
                  ),
                ],
              ),
        body: SafeArea(
  ...
  /// 세로 모드 레이아웃
  Widget _buildPortraitLayout(LightUpState gameState) {
    return Column(
      children: [
          ...
          child: Padding(
            ...
            child: Center(
              child: ConstrainedBox(
                ...
                child: const LightUpBoardWidget(),
              ),
            ),
          ),
        ),
  ...
  /// 가로 모드 레이아웃
  Widget _buildLandscapeLayout(BuildContext context, WidgetRef ref, LightUpState gameState) {
    return Row(
      children: [
        Expanded(
          flex: 5,
          child: Padding(
            ...
            child: Center(child: const LightUpBoardWidget()),
          ),
        ),
        Expanded(
          flex: 4,
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _ToggleButton(
                  label: '💡',
                  isSelected: currentMode == LightUpInputMode.bulb,
                  onTap: () => notifier.setInputMode(LightUpInputMode.bulb),
                  isDark: isDark,
                ),
                const SizedBox(width: 4),
                // X
                _ToggleButton(
                  label: '✕',
                  isSelected: currentMode == LightUpInputMode.crossMark,
                  onTap: () => notifier.setInputMode(LightUpInputMode.crossMark),
                  isDark: isDark,
                ),
                const SizedBox(width: 4),
                // 지우개
                _ToggleButton(
                  ...
                  isSelected: currentMode == LightUpInputMode.erase,
                  onTap: () => notifier.setInputMode(LightUpInputMode.erase),
                  isDark: isDark,
                ),
              ],
            ),
...
/// 일시정지 화면
class _LightUpPauseView extends ConsumerWidget {
  final LightUpState gameState;
  const _LightUpPauseView({required this.gameState});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop) ref.read(lightUpNotifierProvider.notifier).resume();
      },
      child: Scaffold(
        appBar: AppBar(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: () => context.go(AppRoutes.lightUp),
...
/// 결과 화면
class _LightUpResultView extends ConsumerStatefulWidget {
  final LightUpState gameState;
  const _LightUpResultView({required this.gameState});

  @override
  ConsumerState<_LightUpResultView> createState() => _LightUpResultViewState();
}

class _LightUpResultViewState extends ConsumerState<_LightUpResultView> {
  ...
  void _showBadgePopupIfNeeded() {
    ...
    final newBadges = ref.read(lightUpNotifierProvider.notifier).lastNewBadges;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    AppStrings.get('lightUp.result.completed'),
                  ...
                  const SizedBox(height: 16),
                  _StatRow(
                    label: AppStrings.get('lightUp.result.time'),
                    value: timeText,
                  ...
                  _StatRow(
                    label: AppStrings.get('lightUp.result.difficulty'),
                    value: '${gameState.difficulty.label} (${gameState.size}x${gameState.size})',
                    isDark: isDark,
                  ),
                  _StatRow(
                    label: AppStrings.get('lightUp.result.mistakes'),
                    value: '${gameState.mistakeCount}',
                    isDark: isDark,
                  ),
                  _StatRow(
                    label: AppStrings.get('lightUp.result.hints'),
  ...
  Widget _buildNewBadgesSection(WidgetRef ref, BuildContext context) {
    final badges = ref.read(lightUpNotifierProvider.notifier).lastNewBadges;
  ...
  Color _gradeColor(LightUpGrade grade, bool isDark) {
    switch (grade) {
      case LightUpGrade.perfect:
        return Colors.amber.shade600;
      case LightUpGrade.excellent:
        return isDark ? Colors.green.shade300 : Colors.green.shade600;
      case LightUpGrade.great:
        return isDark ? Colors.blue.shade300 : Colors.blue.shade600;
      case LightUpGrade.good:
