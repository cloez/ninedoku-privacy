...
import 'light_up_badge_definitions.dart';

/// 라이트업 배지 서비스 — 달성 평가 및 저장
class LightUpBadgeService {
  static const _keyAcquiredBadges = 'lightup_acquired_badges';
  final SharedPreferences _prefs;

  LightUpBadgeService(this._prefs);
  ...
  List<BadgeDefinition> evaluateNewBadges(List<CompletedGameRecord> records) {
    ...
    for (final badge in lightUpBadgeDefinitions) {
  ...
  List<({BadgeDefinition badge, bool acquired})> getAllBadges() {
    final acquired = getAcquiredBadgeIds();
    return lightUpBadgeDefinitions
