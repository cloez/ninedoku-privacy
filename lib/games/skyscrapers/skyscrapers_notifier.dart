...
import 'skyscrapers_badge_service.dart';
import 'skyscrapers_storage_service.dart';
import 'engine/skyscrapers_board.dart';
import 'engine/skyscrapers_generator.dart';
...
const _storageKey = 'skyscrapers_current_game';

/// Skyscrapers 게임 상태 관리 Notifier
class SkyscrapersNotifier extends StateNotifier<SkyscrapersState?>
    with WidgetsBindingObserver {
  Timer? _timer;
  final SharedPreferences? _prefs;

  /// 완료 기록 저장 서비스
  SkyscrapersStorageService? _storageService;

  /// 배지 평가 서비스
  SkyscrapersBadgeService? _badgeService;

  /// 마지막으로 새로 획득한 배지 목록 (결과 화면 표시용)
  List<BadgeDefinition> lastNewBadges = [];

  SkyscrapersNotifier({SharedPreferences? prefs})
      : _prefs = prefs,
        super(null) {
    if (prefs != null) {
      _storageService = SkyscrapersStorageService(prefs);
      _badgeService = SkyscrapersBadgeService(prefs);
    }
  ...
  void _tryRestore() {
    ...
    try {
      ...
      final saved = SkyscrapersState.fromJson(json);
  ...
  /// 새 게임 시작
  void startNewGame({
    required SkyscrapersGameMode mode,
    required SkyscrapersDifficulty difficulty,
  }) {
    _timer?.cancel();
    lastNewBadges = [];
  ...
  void startDailyPuzzle() {
    ...
    const difficulty = SkyscrapersDifficulty.medium;
  ...
  void _toggleNote(int row, int col, int num) {
    ...
    final undoAction = SkyscrapersUndoAction(
      type: SkyscrapersUndoType.toggleNote,
      row: row,
      col: col,
      previousValue: 0,
  ...
  void _applyValue(int row, int col, int previousValue, int newValue) {
    ...
    final actionType = newValue == 0
        ? SkyscrapersUndoType.clearValue
        : SkyscrapersUndoType.setValue;
    final undoAction = SkyscrapersUndoAction(
      type: actionType,
      row: row,
      col: col,
  ...
  void undo() {
    ...
    SkyscrapersBoard newBoard;

    if (lastAction.type == SkyscrapersUndoType.toggleNote) {
      // 메모 되돌리기
      final idx = lastAction.row * state!.size + lastAction.col;
  ...
  void _checkCompletion() {
    if (state == null) return;
    if (SkyscrapersSolver.isComplete(state!.current)) {
      _timer?.cancel();
...
/// Skyscrapers Provider
final skyscrapersProvider =
    StateNotifierProvider<SkyscrapersNotifier, SkyscrapersState?>((ref) {
  return SkyscrapersNotifier();
});

/// SharedPreferences가 주입된 SkyscrapersNotifier
final skyscrapersNotifierProvider =
    StateNotifierProvider<SkyscrapersNotifier, SkyscrapersState?>((ref) {
  try {
    final prefs = ref.watch(sharedPreferencesProvider);
    return SkyscrapersNotifier(prefs: prefs);
  } catch (_) {
    return SkyscrapersNotifier();
  }
});
