...
import '../widgets/skyscrapers_board_widget.dart';
...
/// Skyscrapers 게임 플레이 화면
class SkyscrapersGameScreen extends ConsumerStatefulWidget {
  const SkyscrapersGameScreen({super.key});

  @override
  ConsumerState<SkyscrapersGameScreen> createState() =>
      _SkyscrapersGameScreenState();
}

class _SkyscrapersGameScreenState extends ConsumerState<SkyscrapersGameScreen> {
  @override
  Widget build(BuildContext context) {
    ...
    if (gameState == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) context.go(AppRoutes.skyscrapers);
      });
    ...
    if (gameState.isCompleted && !gameState.isAutoCompleting) {
      return _SkyscrapersResultView(gameState: gameState);
    }

    // 일시정지 시
    if (gameState.isPaused) {
      return _SkyscrapersPauseView(gameState: gameState);
    }
        ...
        appBar: isLandscape
            ...
            : AppBar(
                ...
                leading: IconButton(
                  ...
                  tooltip: AppStrings.get('skyscrapers.back'),
                ),
                actions: [
                  IconButton(
                    ...
                    tooltip: AppStrings.get('skyscrapers.pause'),
                  ),
                ],
              ),
        body: SafeArea(
  ...
  /// 세로 모드 레이아웃
  Widget _buildPortraitLayout(SkyscrapersState gameState) {
    return Column(
      children: [
          ...
          child: Padding(
            ...
            child: Center(
              child: ConstrainedBox(
                ...
                child: const SkyscrapersBoardWidget(),
              ),
            ),
          ),
        ),
      ...
      children: [
        Expanded(
          ...
          child: Padding(
            ...
            child: const Center(child: SkyscrapersBoardWidget()),
          ),
        ),
        Expanded(
          flex: 4,
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        AppStrings.get('skyscrapers.title'),
...
/// 일시정지 화면
class _SkyscrapersPauseView extends ConsumerWidget {
  final SkyscrapersState gameState;
  const _SkyscrapersPauseView({required this.gameState});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop) ref.read(skyscrapersNotifierProvider.notifier).resume();
      },
      child: Scaffold(
        appBar: AppBar(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: () => context.go(AppRoutes.skyscrapers),
...
/// 결과 화면
class _SkyscrapersResultView extends ConsumerStatefulWidget {
  final SkyscrapersState gameState;
  const _SkyscrapersResultView({required this.gameState});

  @override
  ConsumerState<_SkyscrapersResultView> createState() =>
      _SkyscrapersResultViewState();
}

class _SkyscrapersResultViewState extends ConsumerState<_SkyscrapersResultView> {
  ...
  void _showBadgePopupIfNeeded() {
    ...
    final newBadges =
        ref.read(skyscrapersNotifierProvider.notifier).lastNewBadges;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    AppStrings.get('skyscrapers.result.completed'),
                  ...
                  const SizedBox(height: 16),
                  _StatRow(
                    label: AppStrings.get('skyscrapers.result.time'),
                    value: timeText,
                  ...
                  _StatRow(
                    label: AppStrings.get('skyscrapers.result.difficulty'),
                    value:
                        '${gameState.difficultyLabel} (${gameState.size}x${gameState.size})',
                    isDark: isDark,
                  ),
                  _StatRow(
                    label: AppStrings.get('skyscrapers.result.mistakes'),
                    value: '${gameState.mistakeCount}',
                    isDark: isDark,
                  ),
                  _StatRow(
                    label: AppStrings.get('skyscrapers.result.hints'),
  ...
  Color _gradeColor(SkyscrapersGrade grade, bool isDark) {
    switch (grade) {
      case SkyscrapersGrade.perfect:
        return Colors.amber.shade600;
      case SkyscrapersGrade.excellent:
        return isDark ? Colors.green.shade300 : Colors.green.shade600;
      case SkyscrapersGrade.great:
        return isDark ? Colors.blue.shade300 : Colors.blue.shade600;
      case SkyscrapersGrade.good:
