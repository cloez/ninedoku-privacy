...
import 'minesweeper_badge_definitions.dart';

/// 지뢰찾기 배지 서비스 — 달성 평가 및 저장
class MinesweeperBadgeService {
  static const _keyAcquiredBadges = 'minesweeper_acquired_badges';
  final SharedPreferences _prefs;

  MinesweeperBadgeService(this._prefs);
  ...
  List<BadgeDefinition> evaluateNewBadges(List<CompletedGameRecord> records) {
    ...
    for (final badge in minesweeperBadgeDefinitions) {
  ...
  List<({BadgeDefinition badge, bool acquired})> getAllBadges() {
    final acquired = getAcquiredBadgeIds();
    return minesweeperBadgeDefinitions
