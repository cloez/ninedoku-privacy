...
import 'nonogram_badge_definitions.dart';

/// 노노그램 배지 서비스 — 달성 평가 및 저장
class NonogramBadgeService {
  static const _keyAcquiredBadges = 'nonogram_acquired_badges';
  final SharedPreferences _prefs;

  NonogramBadgeService(this._prefs);
  ...
  List<BadgeDefinition> evaluateNewBadges(List<CompletedGameRecord> records) {
    ...
    for (final badge in nonogramBadgeDefinitions) {
  ...
  List<({BadgeDefinition badge, bool acquired})> getAllBadges() {
    final acquired = getAcquiredBadgeIds();
    return nonogramBadgeDefinitions
