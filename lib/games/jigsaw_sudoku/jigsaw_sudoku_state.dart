...
/// 직소 스도쿠 게임 모드
enum JigsawSudokuGameMode {
  classic('mode.classic'),
  relax('mode.relax'),
  dailyPuzzle('mode.dailyPuzzle'),
  challenge('mode.challenge');

  const JigsawSudokuGameMode(this.labelKey);
...
/// Undo 액션 타입
enum JigsawSudokuUndoType { setValue, clearValue, toggleNote }

/// Undo 스택 항목
class JigsawSudokuUndoAction {
  final JigsawSudokuUndoType type;
  final int row;
  final int col;
  final int previousValue;
  final Set<int>? previousNotes;

  const JigsawSudokuUndoAction({
    required this.type,
    required this.row,
    required this.col,
...
/// 완료 시 등급
enum JigsawSudokuGrade {
  perfect('S', 'grade.perfect'),
  excellent('A', 'grade.excellent'),
  great('B', 'grade.great'),
  good('C', 'grade.good');

  const JigsawSudokuGrade(this.symbol, this.labelKey);
  final String symbol;
  final String labelKey;
  // 현재 언어에 맞는 표시명 (다국어)
  String get label => AppStrings.get(labelKey);
  ...
  /// 등급 산정
  static JigsawSudokuGrade evaluate({
  ...
  /// 난이도별 기준 시간 (초)
  static int baseTimeForDifficulty(JigsawDifficulty difficulty) {
    switch (difficulty) {
...
class JigsawSudokuState {
  ...
  /// 게임 모드
  final JigsawSudokuGameMode mode;
  ...
  /// Undo 스택
  final List<JigsawSudokuUndoAction> undoStack;
  ...
  /// 마지막 힌트 결과
  final JigsawSudokuHintResult? lastHintResult;

  const JigsawSudokuState({
    required this.board,
    required this.mode,
  ...
  /// 등급
  JigsawSudokuGrade get grade => JigsawSudokuGrade.evaluate(
  ...
  /// copyWith
  JigsawSudokuState copyWith({
    JigsawSudokuBoard? board,
    JigsawSudokuGameMode? mode,
    JigsawDifficulty? difficulty,
    int? elapsedSeconds,
    int? mistakeCount,
    int? hintCount,
    ...
    List<JigsawSudokuUndoAction>? undoStack,
    ...
    JigsawSudokuHintResult? lastHintResult,
  ...
  /// JSON 역직렬화
  factory JigsawSudokuState.fromJson(Map<String, dynamic> json) {
    final selectedCellJson = json['selectedCell'] as Map<String, dynamic>?;

    return JigsawSudokuState(
      board: JigsawSudokuBoard.fromJson(
        json['board'] as Map<String, dynamic>,
      ),
      mode: JigsawSudokuGameMode.values.byName(json['mode'] as String),
      difficulty: JigsawDifficulty.values.byName(json['difficulty'] as String),
