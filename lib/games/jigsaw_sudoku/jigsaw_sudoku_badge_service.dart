...
import 'jigsaw_sudoku_badge_definitions.dart';

/// 직소 스도쿠 배지 서비스 — 달성 평가 및 저장
class JigsawSudokuBadgeService {
  static const _keyAcquiredBadges = 'jigsaw_acquired_badges';
  final SharedPreferences _prefs;

  JigsawSudokuBadgeService(this._prefs);
  ...
  List<BadgeDefinition> evaluateNewBadges(List<CompletedGameRecord> records) {
    ...
    for (final badge in jigsawSudokuBadgeDefinitions) {
  ...
  List<({BadgeDefinition badge, bool acquired})> getAllBadges() {
    final acquired = getAcquiredBadgeIds();
    return jigsawSudokuBadgeDefinitions
