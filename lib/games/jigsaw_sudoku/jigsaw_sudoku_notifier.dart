...
import 'jigsaw_sudoku_badge_service.dart';
import 'jigsaw_sudoku_storage_service.dart';
import 'engine/jigsaw_sudoku_generator.dart';
import 'engine/jigsaw_sudoku_hint.dart';
import 'jigsaw_sudoku_state.dart';

import '../../shared/services/sound_manager.dart';
/// SharedPreferences 저장 키
const _storageKey = 'jigsaw_sudoku_current_game';

/// 직소 스도쿠 게임 상태 관리 Notifier
class JigsawSudokuNotifier extends StateNotifier<JigsawSudokuState?>
    with WidgetsBindingObserver {
  Timer? _timer;
  final SharedPreferences? _prefs;

  /// 완료 기록 저장 서비스
  JigsawSudokuStorageService? _storageService;

  /// 배지 평가 서비스
  JigsawSudokuBadgeService? _badgeService;

  /// 마지막으로 새로 획득한 배지 목록 (결과 화면 표시용)
  List<BadgeDefinition> lastNewBadges = [];

  JigsawSudokuNotifier({SharedPreferences? prefs})
      : _prefs = prefs,
        super(null) {
    if (prefs != null) {
      _storageService = JigsawSudokuStorageService(prefs);
      _badgeService = JigsawSudokuBadgeService(prefs);
    }
  ...
  void _tryRestore() {
    ...
    try {
      ...
      final saved = JigsawSudokuState.fromJson(json);
  ...
  /// 새 게임 시작
  void startNewGame({
    required JigsawSudokuGameMode mode,
  ...
  void startDailyPuzzle() {
    ...
    final seed = now.year * 10000 + now.month * 100 + now.day + 1000;
    const difficulty = JigsawDifficulty.medium;
    ...
    state = JigsawSudokuState(
      board: result.board,
      mode: JigsawSudokuGameMode.dailyPuzzle,
      difficulty: difficulty,
    );
  ...
  void _setValue(int row, int col, int value) {
    ...
    // Undo 기록
    final undoAction = JigsawSudokuUndoAction(
  ...
  void _clearCell(int row, int col) {
    ...
    final undoAction = JigsawSudokuUndoAction(
      type: JigsawSudokuUndoType.clearValue,
      row: row,
      col: col,
      previousValue: currentValue,
    );
  ...
  void deleteSelected() {
    ...
    if (state!.board.cells[row][col] != 0) {
      ...
      // 메모도 삭제
      final undoAction = JigsawSudokuUndoAction(
        type: JigsawSudokuUndoType.toggleNote,
        row: row,
        col: col,
        previousValue: 0,
        previousNotes: Set<int>.from(state!.board.notes[row][col]),
  ...
  void undo() {
    ...
    switch (lastAction.type) {
      case JigsawSudokuUndoType.setValue:
      case JigsawSudokuUndoType.clearValue:
      ...
      case JigsawSudokuUndoType.toggleNote:
        if (lastAction.previousNotes != null) {
          final restoredNotes = List.generate(
            9,
            (r) => List.generate(
  ...
  void getHint() {
    ...
    final hint = JigsawSudokuHintEngine.getHint(state!.board, nextLevel);
    if (hint == null) return;
...
/// 직소 스도쿠 Provider
final jigsawSudokuProvider =
    StateNotifierProvider<JigsawSudokuNotifier, JigsawSudokuState?>((ref) {
  return JigsawSudokuNotifier();
});

/// SharedPreferences가 주입된 JigsawSudokuNotifier
final jigsawSudokuNotifierProvider =
    StateNotifierProvider<JigsawSudokuNotifier, JigsawSudokuState?>((ref) {
  try {
    final prefs = ref.watch(sharedPreferencesProvider);
    return JigsawSudokuNotifier(prefs: prefs);
  } catch (_) {
    return JigsawSudokuNotifier();
  }
});
