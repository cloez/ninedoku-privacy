...
import '../widgets/star_battle_board_widget.dart';
...
/// Star Battle 게임 플레이 화면
class StarBattleGameScreen extends ConsumerStatefulWidget {
  const StarBattleGameScreen({super.key});

  @override
  ConsumerState<StarBattleGameScreen> createState() => _StarBattleGameScreenState();
}

class _StarBattleGameScreenState extends ConsumerState<StarBattleGameScreen> {
  @override
  Widget build(BuildContext context) {
    ...
    if (gameState == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) context.go(AppRoutes.starBattle);
      });
    ...
    if (gameState.isCompleted && !gameState.isAutoCompleting) {
      return _StarBattleResultView(gameState: gameState);
    }

    // 일시정지 시
    if (gameState.isPaused) {
      return _StarBattlePauseView(gameState: gameState);
    }
        ...
        appBar: isLandscape
            ...
            : AppBar(
                ...
                leading: IconButton(
                  ...
                  tooltip: AppStrings.get('starBattle.back'),
                ),
                actions: [
                  IconButton(
                    icon: const Icon(Icons.pause_rounded),
                    onPressed: () => ref.read(starBattleNotifierProvider.notifier).pause(),
                    tooltip: AppStrings.get('starBattle.pause'),
                  ),
                ],
              ),
        body: SafeArea(
  ...
  /// 세로 모드 레이아웃
  Widget _buildPortraitLayout(StarBattleState gameState) {
    return Column(
      children: [
          ...
          child: Padding(
            ...
            child: Center(
              child: ConstrainedBox(
                ...
                child: const StarBattleBoardWidget(),
              ),
            ),
          ),
        ),
  ...
  /// 가로 모드 레이아웃
  Widget _buildLandscapeLayout(BuildContext context, WidgetRef ref, StarBattleState gameState) {
    return Row(
      children: [
        Expanded(
          flex: 5,
          child: Padding(
            ...
            child: Center(child: const StarBattleBoardWidget()),
          ),
        ),
        Expanded(
          flex: 4,
              ...
              children: [
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        AppStrings.get('starBattle.title'),
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Text(
            '${gameState.difficulty.label} (${gameState.size}x${gameState.size}, ${gameState.starCount}-star)',
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                _ToggleButton(
                  label: '★',
                  isSelected: currentMode == StarBattleInputMode.star,
                  onTap: () => notifier.setInputMode(StarBattleInputMode.star),
                  isDark: isDark,
                ),
                const SizedBox(width: 4),
                // ✕
                _ToggleButton(
                  label: '✕',
                  isSelected: currentMode == StarBattleInputMode.cross,
                  onTap: () => notifier.setInputMode(StarBattleInputMode.cross),
                  isDark: isDark,
                ),
                const SizedBox(width: 4),
                // 지우개
                _ToggleButton(
                  ...
                  isSelected: currentMode == StarBattleInputMode.erase,
                  onTap: () => notifier.setInputMode(StarBattleInputMode.erase),
                  isDark: isDark,
                ),
              ],
            ),
...
class _ToggleButton extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final selectedBg = isDark ? Colors.amber.shade700 : Colors.amber.shade100;
    final unselectedBg = Colors.transparent;
    final selectedFg = isDark ? Colors.white : Colors.amber.shade800;
...
/// 일시정지 화면
class _StarBattlePauseView extends ConsumerWidget {
  final StarBattleState gameState;
  const _StarBattlePauseView({required this.gameState});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (didPop, _) {
        if (!didPop) ref.read(starBattleNotifierProvider.notifier).resume();
      },
      child: Scaffold(
        appBar: AppBar(
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      onPressed: () => context.go(AppRoutes.starBattle),
...
/// 결과 화면
class _StarBattleResultView extends ConsumerStatefulWidget {
  final StarBattleState gameState;
  const _StarBattleResultView({required this.gameState});

  @override
  ConsumerState<_StarBattleResultView> createState() => _StarBattleResultViewState();
}

class _StarBattleResultViewState extends ConsumerState<_StarBattleResultView> {
  ...
  void _showBadgePopupIfNeeded() {
    ...
    final newBadges = ref.read(starBattleNotifierProvider.notifier).lastNewBadges;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    AppStrings.get('starBattle.result.completed'),
                  ...
                  const SizedBox(height: 16),
                  _StatRow(
                    label: AppStrings.get('starBattle.result.time'),
                    value: timeText,
                  ...
                  _StatRow(
                    label: AppStrings.get('starBattle.result.difficulty'),
                    value: '${gameState.difficulty.label} (${gameState.size}x${gameState.size}, ${gameState.starCount}-star)',
                    isDark: isDark,
                  ),
                  _StatRow(
                    label: AppStrings.get('starBattle.result.mistakes'),
                    value: '${gameState.mistakeCount}',
                    isDark: isDark,
                  ),
                  _StatRow(
                    label: AppStrings.get('starBattle.result.hints'),
  ...
  Widget _buildNewBadgesSection(WidgetRef ref, BuildContext context) {
    final badges = ref.read(starBattleNotifierProvider.notifier).lastNewBadges;
  ...
  Color _gradeColor(StarBattleGrade grade, bool isDark) {
    switch (grade) {
      case StarBattleGrade.perfect:
        return Colors.amber.shade600;
      case StarBattleGrade.excellent:
        return isDark ? Colors.green.shade300 : Colors.green.shade600;
      case StarBattleGrade.great:
        return isDark ? Colors.blue.shade300 : Colors.blue.shade600;
      case StarBattleGrade.good:
