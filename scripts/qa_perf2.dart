// yin_yang, tents, nonograms 성능 측정 (killer 제외 — 별도 측정)
...
void main() {
  ...
  for (final d in TentsDifficulty.values) {
    ...
    for (var seed = 1; seed <= 5; seed++) {
      final sw = Stopwatch()..start();
      final r = TentsGenerator.generate(size: d.gridSize, difficulty: d.code, seed: seed);
      sw.stop();
      final ms = sw.elapsedMilliseconds;
      sum += ms;
      if (ms > max) max = ms;
  ...
  for (final d in NonogramDifficulty.values) {
    ...
    for (var seed = 1; seed <= 5; seed++) {
      final sw = Stopwatch()..start();
      final r = NonogramGenerator.generate(size: d.gridSize, seed: seed, difficulty: d.code);
      sw.stop();
      final ms = sw.elapsedMilliseconds;
      sum += ms;
      if (ms > max) max = ms;
